...
DISCORD_REDIRECT_URI = 'https://edfflipping-backend.onrender.com/callback'  # Pas aan als je online host
...
def millionaires():
    ...
    if not user_has_access(session.get('roles', []), ACCESS_MILLIONAIRES):
        return abort(403)
    # Laad de Excel als DataFrame
    df = pd.read_excel('millionaires.xlsx')  # of pd.read_csv('millionaires.csv')
    # Zet om naar lijst van rijen voor de template
...
def billionaires():
    ...
    if not user_has_access(session.get('roles', []), ACCESS_BILLIONAIRES):
        return abort(403)
    return render_template("billionaires.html")
...
def trillionaires():
    ...
    if not user_has_access(session.get('roles', []), ACCESS_TRILLIONAIRES):
        return abort(403)
    return render_template("trillionaires.html")
